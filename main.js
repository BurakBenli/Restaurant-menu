...
const renderButtons = (active) => {
  ...
  buttonsData.forEach((btn) => {
    ...
    const buttonEkle = document.createElement("button");
    //* butonlara classlarını ekleme
    buttonEkle.className = "btn btn-outline-dark filter-btn";
    //* İçerisideki yazıyı değiştirme
    buttonEkle.textContent = btn.text;
    //* Hangi kategori olduğu bilgisini buton elementine ekleme
    buttonEkle.dataset.category = btn.value;
    //* Eğer ki active kategorisiyle buton eşleşirse ona farklı class ekle
    if (btn.value === active) {
      buttonEkle.classList.add("bg-dark", "text-light");
    }
    //* HTML'e gönderme
    elements.buttonsArea.appendChild(buttonEkle);
  });
};
